...
def build_dry_run_plan(prepared_npz: Path, input_summary: Path, config_path: Path, gpu_device: str | None) -> dict:
    ...
    return {
        ...
        "working_matrix_yx": [config.working_matrix, config.working_matrix],
        "crop_matrix_yx_after_native_solver": list(config.crop_matrix_yx),
        ...
        "scale_estimation": {"frames": scale_frame_indices(config.num_recon_frames).tolist(), "range": "5:50", "command": "nufft -g -x 360:360:1 -a", "usc_conditional": "p90 if (max-p90) < 2*(p90-median) else max"},
        ...
        "future_outputs": ["sens_map.npy", "sens_map_magnitude.png", "tfd_bart_native_complex.npy", "tfd_bart_native_magnitude.npy", "center-cropped [213,240] arrays"],
    }
